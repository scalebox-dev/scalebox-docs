...
- **狀態欄位**：列表與詳情回應可能包含 `status`（例如 `active`）以供後續擴充；**無法**透過 `PUT /v1/projects/{project_id}` 修改。回應中**不包含**專案層級的消費匯總或 `total_spent`。
...
變更類操作現在會同時回傳結構化 `data` 與頂層 `message`。
...
更新專案名稱、描述或預設專案標記。專案 `status` 由系統保留，**無法**透過此 API 修改。
...
這個回應現在與 list/get 維持一致的投影結構，並另外在頂層提供成功 `message`。
...
|------|------|------|------|
| page | integer | 否 | 分頁頁碼，預設 `1` |
| limit | integer | 否 | 每頁回傳筆數，預設 `50`；別名：`page_size` |
| offset | integer | 否 | 要略過的筆數，預設 `0`；別名：`skip` |
...
此端點會回傳 `data.sandboxes`，並在 `data.pagination` 提供標準分頁資訊。
