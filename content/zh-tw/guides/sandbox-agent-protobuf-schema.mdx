...
description: 面向進階直連整合場景公開的 sandbox-agent protobuf 協議定義。
icon: FileCode
---

<Callout type="warn">
此頁面面向需要直接使用原始 protobuf 合約的進階整合方。對大多數正式整合場景，我們仍強烈建議優先使用官方 SDK，因為它們已經封裝了認證、ConnectRPC 呼叫約定與串流處理細節。
</Callout>

本頁發布目前 direct sandbox-domain 整合路徑所使用的 sandbox-agent protobuf schema。它是 procedure 名稱、請求與回應訊息、列舉以及串流事件結構的來源合約。
...
- [直連 sandbox-agent API](/zh-tw/guides/direct-sandbox-agent-api)
