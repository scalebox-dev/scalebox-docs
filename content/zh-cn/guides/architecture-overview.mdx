...
ScaleBox 提供**隔离的沙盒环境**：每个沙盒具有明确的 CPU、内存、存储、网络以及可选能力（例如暴露端口或 WebRTC）。你通过模板、项目与 API 使用沙盒——**无需**接触底层编排概念。**控制面**（REST API、持久化、调度、计费等）接收你的请求，为你创建与管理沙盒，并将状态与用量反馈给你。
...
- **数据面** — 沙盒在 ScaleBox 托管的计算基础设施上运行。创建或更新沙盒会按你的模板与资源配置生效；底层实现不在产品界面中暴露，也无需你配置。
...
**REST API** 是自动化与集成的契约。任何能发 HTTPS 请求并携带 API Key（例如 `X-API-Key` 头）的语言或工具都可以直接使用。它现在已不仅限于高层生命周期操作，而是支持大部分端到端沙盒能力：资源管理、文件操作、终端访问，以及代理式的沙盒命令执行与流式输出。

**适合：** CI/CD、内部工具、按需在服务中创建和管理沙盒，以及需要对沙盒资源与沙盒内操作进行精细程序化控制的场景。
...
**与 API 的关系：** 每条命令对应具体的 API 调用，而 CLI 现在已经覆盖了大部分已文档化、面向客户开放的后端 API。安装、认证与命令说明见 [CLI 参考](/zh-cn/cli/)。
...
flowchart TB
  ...
  subgraph control [ScaleBox 控制面]
    API[版本化 REST API<br/>认证、模板、项目、沙盒、用量、计费]
    Services[认证、生命周期编排、<br/>资源管理、用量与计费]
  end

  subgraph sandbox [单个沙盒运行侧]
    Runtime[沙盒运行时<br/>计算、存储、网络、端口]
    Agent[沙盒 Agent 能力<br/>文件、终端、exec、run-code*]
  end
  ...
  API -->|控制面请求| Services
  Services -->|创建、暂停、恢复、终止| Runtime
  Runtime -->|沙盒内 Agent| Agent
...
4. 沙盒运行后，你可以继续通过终端访问、端口转发、文件操作、代理命令执行，或在模板支持时进行 notebook 风格的代码执行来使用它。
