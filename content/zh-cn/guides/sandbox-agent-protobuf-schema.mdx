...
description: 面向高级直连集成场景公开的 sandbox-agent protobuf 协议定义。
icon: FileCode
---

<Callout type="warn">
此页面面向需要直接使用原始 protobuf 合约的高级集成方。对绝大多数生产集成场景，我们仍强烈建议优先使用官方 SDK，因为它们已经封装了认证、ConnectRPC 调用约定与流式处理细节。
</Callout>

本页发布当前 direct sandbox-domain 集成路径所使用的 sandbox-agent protobuf schema。它是 procedure 名称、请求与响应消息、枚举以及流事件结构的源合约。
...
- [直连 sandbox-agent API](/zh-cn/guides/direct-sandbox-agent-api)
