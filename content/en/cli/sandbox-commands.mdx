...
Single-sandbox resume supports an optional OSS delta via `--object-storage` (JSON with `remove_all`, `disable_all`, or `mounts[]`). Not supported when resuming multiple IDs.
