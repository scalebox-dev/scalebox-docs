...
In the audited `scalebox-cli` source, `template build` currently registers `-p` for `--path`, which conflicts with the global `-p` shorthand for `--profile`. Invoking `template build` currently panics before execution in this source snapshot, so the table below documents the implemented flags but the command itself needs a CLI fix.
</Callout>
...
| `--path` | string | Build context path (default: current directory; the `-p` shorthand currently conflicts with global `--profile`) |
...
| `--visibility` | string | `private`, `account_shared`, or `public` (admin only); when omitted, non-admins default to `private` and admins default to `public` |
...
Unlike `template create-custom`, `template import` requires the source image to already define an Entrypoint or Cmd; passing `--custom-command` does not bypass that prevalidation. `template import` does not accept `--base` and always creates a root custom template. Use `template import-jobs` to inspect backend-side remote imports.
...
Only the template owner or an admin can share it, and the template must already be active.
...
Only the template owner or an admin can unshare it, and the template must already be active. The CLI also notes that the image is copied to your private Harbor project during this operation.
...
- Regular users can list accessible templates, inspect them, derive new templates, create custom templates, import images, and update templates they own.
...
- `template build` defaults visibility to `private` for non-admin users and `public` for admins when `--visibility` is omitted.
- `template create-custom` and `template import` default visibility to `private`.
...
- In the audited source snapshot, `template build` currently panics because the build-local `-p` shorthand conflicts with the global `-p/--profile` flag
