...
- **New users:** If you have no active API keys, browser login can create one and return it to the CLI once
- **Existing keys:** Browser login cannot retrieve a key already on the server—use a saved local key (`~/.scalebox-cli/config.json`), pass `--api-key`, or create a new key in the dashboard and copy it immediately

<Callout type="info">
Use `--no-browser` to skip browser authentication and fall back to other methods. Full API key secrets are shown only once at creation; the server stores a hash and display prefix only.
</Callout>
...
- **Browser login failed with existing keys:** Delete or disable the old key, create a new one in the dashboard, or use `--api-key` with a key you already saved locally
