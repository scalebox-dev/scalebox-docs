---
title: Sandbox-agent protobuf schema
description: Published protobuf contract for advanced direct integrations with the sandbox agent.
icon: FileCode
---

<Callout type="warn">
This page is for advanced integrators who need the raw protobuf contract. For most production integrations, we still strongly recommend the official SDKs because they already encapsulate authentication, ConnectRPC calling conventions, and stream handling.
</Callout>

This page publishes the current sandbox-agent protobuf schema used by the direct sandbox-domain integration path. It is the source contract for procedure names, request and response messages, enums, and stream event structures.
...
- [Direct sandbox-agent API](/en/guides/direct-sandbox-agent-api)
