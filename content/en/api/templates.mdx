...
Returns JSON in the standard success envelope:
...
Same access rules as the JSON endpoint. Responds with `Content-Type: text/markdown; charset=utf-8` and `Content-Disposition: attachment` for a `.md` file. Returns **400** if there is no document or the stored content is empty/whitespace-only (avoids useless downloads).
