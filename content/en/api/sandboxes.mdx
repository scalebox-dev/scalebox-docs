...
- `deadline`: RFC3339Nano UTC string (hard wall clock limit)
...
This endpoint uses a `token` query parameter because **EventSource cannot send custom headers**. Use the same access token you would otherwise send as `Authorization: Bearer <token>`.
